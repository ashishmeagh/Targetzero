        ...
        <div class="card-body p-t-15">
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'attribute' => 'report_topic',
                        'format' => 'html',
                        'value' => function($data)
                        {
                            return Html::a( $data->report_topic, ['/report-topic/update?id='.$data->id]);
                        },
                        'label' => 'Report Topic',
                    ],
                    [
...
<script>
	...
	var deleteItem = function(item_id){
		...
		swal({ 
			...
			if(isConfirm){
                executeAjax("<?= Yii::$app->urlManager->createUrl('report-topic/delete?id=') ?>"+item_id).done(function(r){
					if(r){
