    ...
    public function actionIndex() {
        ...
        if (Yii::$app->session->get("user.logged") === "true") {
            ...
            if (Yii::$app->request->post()) {
                ...
                $user = Yii::$app->db->createCommand("SELECT * FROM [user] WHERE user_name= '$username' AND password = '$password' AND is_active = 1 AND role_id not in (10,11,12,13,14) and IsAduser = 0 ")->queryOne();
...
//                        Yii::$app->getSession()->setFlash( 'error', 'Your Text Here..' );
    ...
    public function actionLogout() {
        ...
        unset($_COOKIE['rememberMe']);
        if ($Adlogin) {
            $this->redirect(array('saml/logout'));
        } else {
