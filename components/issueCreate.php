    ...
    static function createViolation( $user, $affectedUser, $post )
    {
        ...
        $gmtUpdatetimeformat = $gmtDate->format( 'Y/m/d H:i:s' );
         ...
         if($isIssueexist)
         {
             ...
             $response = array(
                    'success' => FALSE,
                    'error' => "CREATE_ERR",
                    'description' => "Issue already exist",
                );
        ...
        try
        {
           ...
           // $appCase->app_case_status_id = APP_CASE_STATUS_OPEN;      
            ...
            $offenderuserandissues = $searchModel->CheckRepeatoffenderissues($affectedUser[ "id" ],$jobsite_id);
            $reptoffeder = (count($offenderuserandissues) > 0) ? true: false;
            
            if ( isset( $user[ "device_type" ] ) && $user[ "device_type" ] != "null" )
            {
                if($user[ "device_type" ] == 'ios'){
                    $appCase->platform_id = 2;
                }elseif($user[ "device_type" ] == 'android'){
                    $appCase->platform_id = 3;
                }else {
                    $appCase->platform_id = 4;
                }
            }else {
                $appCase->platform_id = 4;
            }
          ...
          //  print_r($appCase->getErrors());
            ...
            if ( $se_puede && isset($post[ "photo" ])):
             ...
             if( isset($filepath) && isset($post['photo']) )
                {
                         notification::notifyNewForMobileAttach( $lastInsertID, true, $filepath,$reptoffeder);
               }
               else
               {
                           notification::notifyNew($lastInsertID, true,true,$reptoffeder);
               }
...
// error_log($message, 3, "debugging-messages.log");
    ...
    static function Attachment($user,$post)
    {
             ...
             try
            {
                    if(!(substr($post['photo'],0,5) == 'https') && isset($post['photo'])){
                        $transaction = Yii::$app->db->beginTransaction();
               $binary = base64_decode( $post['photo'] );
                ...
                fwrite( $file, $binary );
                fclose( $file );
                        $filetoUpload = "../web/files/" . $filename . ".jpg";
                        $blobName = $filename . ".jpg";
                        $blobStatus = attachment::uploadBlob($filetoUpload, $blobName);
                ...
                $content->type = "blob";
                $content->file = $blobStatus['destinationURL'];
                $content->save();
                $transaction->commit();
                ...
                $response = array(
                    'success' => TRUE,
                    'id' =>  $content->id,
                        'file_url' => $blobStatus['destinationURL'],
                        'owner_id' => $user['id']
                    );
                        }else{
                            $response = array(
                        ...
                        $post['photo'] = str_replace(' ', '', $post['photo']);
                        $photo_urls = explode(',',$post['photo']);
                        
                        $count = 0;
                        $content_ids = array();
                foreach($photo_urls as $photo_url){
                    $transaction = Yii::$app->db->beginTransaction();
            //         $orig_filename=explode("/",$photo_url);
            //         $split_filename=explode("_",$orig_filename[4]);
            //         $act_filename = $split_filename[0];
            //         $forext_file = $split_filename[1];
            //         $ext_filename = explode(".",$forext_file);
            //         $file_ext = $ext_filename[1];

                    
            // $origin_file = $act_filename.".".$file_ext;
            // $filetoUpload = "../web/files/" . $origin_file;
            // $isFileExist = file_exists($filetoUpload);
            // echo"<pre>";
            // echo var_dump($isFileExist );
            // echo"</pre>";
            // exit("Hello");
                        $sub_str = substr($photo_url,0,5);
                        $content = new Content();
                        $content->is_active = 1;
                        $content->uploader_id = $user['id'];
                        $content->created = date( 'Y/m/d H:i:s' );
